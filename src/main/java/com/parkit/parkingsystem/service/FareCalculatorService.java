	...
	public double calculateFare(Ticket ticket, int nombreOccurenceTicket) {
		if ((ticket.getOutTime() == null) || (ticket.getOutTime().before(ticket.getInTime()))) {
			throw new IllegalArgumentException(
					"Out time provided is incorrect:" + ticket.getOutTime().toString() + ticket.getInTime().toString());
		}

		double inHour = ticket.getInTime().getTime();
		double outHour = ticket.getOutTime().getTime();
		// TODO: Some tests are failing here. Need to check if this logic is correct =>
		// Changed "getHours" with "getTime" which isn't deprecated and also calculate
		// minutes
